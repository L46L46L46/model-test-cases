...
- Функция VisibleFunction для каждой шлюзовой станции определяет список КА, которые могут быть обнаружены ею и вычисляет для них ориентацию антенны (azimuth-elevation), расстояние и допплеровский сдвиг частоты.
- Вывод значений функции осуществляется в json-файл gatewaysTest.json. Файл содержит пару аппарат-шлюзовая станция в случае, если шлюзовая станция может обнаружить аппарат, и параметры для этой пары.
- Функция Visualization, которая выводит значения, полученные функцией VisibleFunction в более удобном формате. Есть возможность передать в функцию номера станции и аппарата и получить для них ответ, обнаруживает ли станция аппарат на данной эпохе, а так же список требуемых значений.
